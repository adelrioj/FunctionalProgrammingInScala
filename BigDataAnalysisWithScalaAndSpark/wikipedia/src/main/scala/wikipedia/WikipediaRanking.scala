...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    //seqOp
    (acc, article) => if(articleContains(article, lang)) acc + 1 else acc,
    //combOp
    (acc1, acc2) => acc1 + acc2
  )
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article =>langs.withFilter(lang => articleContains(article, lang)).map(lang => (lang, article))).groupByKey
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    //similar to makeIndex, using for comprehensions
    val pairs = for {
      article <- rdd
      lang <- langs
      if articleContains(article, lang)
    } yield {
      (lang, 1)
    }

    pairs.reduceByKey(_ + _).sortBy(-_._2).collect.toList
  }

  private def articleContains(article: WikipediaArticle, lang: String) = {
    article.text.split(" ").contains(lang)
  }
