...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'rankLangs' test using wikipedia DAT file") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val ranked: List[(String, Int)] = rankLangs(langs, wikiRdd)
    assert(ranked.head._1 == "JavaScript")
  }
  ...
  test("'makeIndex' works on WikipediaData") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "Ruby", "C")
  ...
  test("'rankLangsUsingIndex' test using wikipedia DAT file") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val ranked: List[(String, Int)] = rankLangsUsingIndex(makeIndex(langs, wikiRdd))
    assert(ranked.head._1 == "JavaScript")
  }
  ...
  test("'rankLangsReduceByKey' test using wikipedia DAT file") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "Ruby")
    val ranked: List[(String, Int)] = rankLangsReduceByKey(langs, wikiRdd)
