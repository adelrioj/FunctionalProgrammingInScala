...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(tag => tweet.text.contains(tag)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.exists(tag => tweet.text.contains(tag)))
